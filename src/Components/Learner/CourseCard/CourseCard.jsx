...
function CourseCard({course}) {
    ...
    const CourseImage = ({ src, alt }) => (
        <div className='overflow-hidden w-72 h-48'>
          <img 
            className='w-full h-full transform transition duration-300 ease-in-out hover:scale-110' 
            src={src} 
            alt={alt} 
          />
        </div>
    ...
    const CourseInfo = ({ category, price, title , color}) => (
        <div className='flex flex-col justify-between items-between w-72 h-32 border-t-0 border-2 border-gray-50 hover:bg-orange-50'>
          <div className='flex justify-between items-center'>
            <p className={`bg-${color}-200 font-inter text-${color}-700 text-xs pt-1 px-2.5 mx-4 my-2`}>{category}</p>
  ...
  return (
    <a href={course.link} className='hover:cursor-pointer'>
        <div className='flex flex-col justify-center items-center drop-shadow-lg bg:white hover:bg-orange-50'>
